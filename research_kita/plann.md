...
由于作者后续对自己的模型进行了改进，这里进行贝叶斯网络构建使用的手法主要参考Kita[4]中提到的优化模型的手法（下面插入Kita[4]的`Fig. 1.`）。在贝叶斯网络进行训练时，训练数据中不仅会考虑过去模型的预测误差，还会同时增加市场模型预测的趋势作为训练数据。

> 已经看过Kita[4]了，会把以往手法说明一下的
>
...
4. **将(3)得到的离散值以及环境分析模型预测的趋势值作为结点构造贝叶斯网络$B'$**
5. 使用$B'$进行股价回报的预测并得到值作为预测结果。

为了与原模型进行对照，在模型的验证阶段将会选择`Toyota汽车`等其他真实股票的数据作为训练数据进行模型的性能评估。
